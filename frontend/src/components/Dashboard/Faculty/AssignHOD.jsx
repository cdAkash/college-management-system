...
export default function AssignHOD() {
  const [formData, setFormData] = useState({ email: "", deptId: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/v1/admin/assign-hod', {
        method: 'PATCH',
        headers: {
      ...
      <div className="w-full max-w-4xl bg-white dark:bg-gray-800 p-6 shadow-md rounded-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Button
            ...
            disabled={isLoading}
          >
            {isLoading ? <Spinner className="w-5 h-5 animate-spin" /> : "Assign HOD"}
          </Button>
        </form>
      </div>
    </div>
